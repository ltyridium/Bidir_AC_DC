...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#warning //When after use STM32CUBEMX to update the project, please make sure that the ISR is correctly placed in CCMRAM
...
void ADC1_2_IRQHandler(void)
{
  ...
  if (LL_ADC_IsActiveFlag_JEOS(ADC1) != 0)
  {
    ...
    if (PFC_systemState.enum_systemState == systemState_CalOffset)
    {
      PFC_HAL_calADCOffset();
    } else
    {
...
void DMA2_Channel5_IRQHandler(void)
{
  ...
  if (LL_DMA_IsActiveFlag_TC5(DMA2))
  {
    ...
    PFC_HAL_debugWriteDoneCallback();
...
void ADC4_IRQHandler(void)
{
  ...
  if (LL_ADC_IsActiveFlag_JEOC(ADC4) != 0)
  {
    /* Clear flag ADC group regular end of unitary conversion */
    LL_ADC_ClearFlag_JEOC(ADC4);
    /* Call interruption treatment function */
